    ...
    module CLI
        ...
        class MainJenkins < Thor
            ...
            option :seed, desc: 'a YAML file containing seed configuration',
                type: :string
            option :status_publisher, desc: 'a job that would publish the CI status generated by the generated status build',
            ...
            def init(url, *package_names)
                ...
                ops.create_or_update_buildconf_job(
                    *package_names,
                    ...
                    dev: options[:dev],
                    status_publisher: options[:status_publisher])
                if options[:trigger]
            ...
            option :publisher, desc: 'the job that will publish status',
                type: :string, default: nil
            def update_status(url, *job_names)
                require 'autoproj/cli/jenkins'
                ops = create_ops(url, target_os: options[:target_os])
                ops.create_or_update_status_job(*job_names, publisher: options[:publisher])
            end
